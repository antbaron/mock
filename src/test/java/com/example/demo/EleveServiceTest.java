...
public class EleveServiceTest {
	...
	public void test1() {
		...
		Mockito.doReturn(expected).when(eleveRepository).findMean("Lucas Leroux");

		Mockito.doReturn(true).when(sut).isPositiveNote(note);
		
		// Act
		int response = sut.lucleroux(note);
		
		// Assert
		Assertions.assertThat(response).isEqualTo(expected);
		
		Eleve e = acEleve.getValue();
		Assertions.assertThat(e.getName()).isEqualTo("Lucas Leroux");
